            ...
            for (var i = 0; i < outputs.length; ++i) {
                ...
                if (defined(outputs[i].Data.LiteralData)) {
                    ...
                    if (outputs[i].Data.ComplexData.mimeType === 'text/csv' || outputs[i].Data.ComplexData.mimeType === 'application/json')
                    {
                        ...
                        var chartVars = '';
                        if (defined(outputs[i].chartvars)) {
                            chartVars = outputs[i].chartvars;
                        }
                        content = '<collapsible title="' + outputs[i].Title + '" open="' + (i === 0 ? 'true' : 'false') + '">';
                        content += '<chart ' + colors + chartVars + ' can-download="true" hide-buttons="false" title="'
                                + outputs[i].Title + '" data=\'' + outputs[i].Data.ComplexData + '\' styling="feature-info"></chart>';
                        content += '</collapsible>';
                    }
                ...
                (outputs || []).reduce(function(previousValue, output) {
                    ...
                    if (defined(output.Data.LiteralData)) {
                        ...
                        if (output.Data.ComplexData.mimeType === "text/csv" || output.Data.ComplexData.mimeType === "application/json")
                        {
                            ...
                            content = '<chart ' + colors + chartVars + ' can-download="true" hide-buttons="false" title="' + output.Title + '" data=\'' + output.Data.ComplexData + '\' styling: "feature-info">';
                        }
